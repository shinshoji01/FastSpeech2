...
class Dataset(Dataset):
    def __init__(
        self, filename, preprocess_config, train_config, sort=False, drop_last=False, ifbert=False, ifms=False,
    ):
    ...
    def __getitem__(self, idx):
        ...
        if self.ifms:
            el_path = os.path.join(
                self.preprocessed_path,
                "EmotionLabels",
                f"{basename}_{self.label_type}.npy",
            )
            el = np.load(el_path, allow_pickle=True).item()
            el = int(np.load(el_path))
        else:
            el = 99
    ...
    def reprocess(self, data, idxs):
        ...
        els = [data[idx]["el"] for idx in idxs]
